//  LINK TO VIEW -> https://youtu.be/mbsmsi7l3r4  

//npm install jsonwebtoken --save 

// ----REFRESH TOKEN - SETTING SHORT EXPIRATION DATE SO IF SOMEONE HAS THAT TOKEN THEN HE CAN'T USE IT AND CREATE PROBLEM FOR LONG
...
app.post('/login',authentication ,  (req, res) => {      // as we want to send token , so post makes more sense
    ...
    const accesstoken = jwt.sign(user, process.env.ACCESS_TOKEN_SECRET , { expires: '15s' });        // we can also add expiration date to token , if we have new token generating system 
    res.json( { accesstoken: accesstoken });
    /*
    after 15s the token will expiree then we need to set another token to work with , usually preferred long time - 10m or 15min
